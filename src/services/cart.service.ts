...
export const cartService = {
  ...
  addToCart: (payload: { product_variant_id: number; quantity: number }) => {
    return axiosClient.post("/v1/cart/items", payload);
  },

  // 3. Cập nhật số lượng (PUT) - Endpoint: /items/{item_id}
  updateQuantity: (itemId: number, quantity: number) => {
    return axiosClient.put(`/v1/cart/items/${itemId}`, { quantity });
  },
  ...
  updateSelection: (itemIds: number[], selected: boolean) => {
    return axiosClient.put("/v1/cart/select", {
      item_ids: itemIds,
      selected: selected,
    });
  },
  ...
  removeItem: (itemId: number) => {
    return axiosClient.delete(`/v1/cart/items/${itemId}`);
  },

  // 6. Xóa tất cả/Làm trống giỏ (DELETE) - Endpoint: /clear
  clearCart: () => {
    return axiosClient.delete("/v1/cart/clear");
  },
};