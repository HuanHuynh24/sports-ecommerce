...
export default function ProductDetailView({ product }: { product: ProductDetail }) {
  ...
  const currentVariant = useMemo(() => {
    return product.variants.find((v) => {
      return Object.entries(selectedAttrs).every(([selectedName, selectedValue]) => {
    ...
    return (
      <div key={attrName} className="mb-4">
        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2 block">
          {attrName}: <span className="text-black dark:text-white font-bold">{selectedAttrs[attrName]}</span>
        </span>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
          {[product.thumbnail, ...product.images.map((img) => img.url)].map((img, idx) => (
            <button
              ...
              className={`relative w-20 h-20 shrink-0 rounded-xl border-2 overflow-hidden bg-white dark:bg-[#120707] p-1 ${activeImage === img ? "border-[var(--color-primary)]" : "border-transparent"}`}
            >
      ...
      <div className="lg:col-span-5 flex flex-col h-full">
        ...
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs font-bold uppercase text-[var(--color-primary)] tracking-wider">{product.brand?.name}</span>
          <span className="text-gray-300">|</span>
          <span className="text-xs text-gray-500">{product.category?.name}</span>
        </div>
        <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 dark:text-white leading-tight mb-2">{product.name}</h1>
        ...
        <div className="flex items-end gap-3 mb-8 pb-8 border-b border-gray-100 dark:border-white/10">
          <span className="text-3xl font-black text-[var(--color-primary)]">{formatCurrency(displayPrice)}</span>
          {product.original_price > displayPrice && (
            <span className="text-lg text-gray-400 line-through font-medium mb-1">{formatCurrency(product.original_price)}</span>
          )}
        </div>
        ...
        <div className="mt-auto">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex items-center border border-gray-300 dark:border-white/20 rounded-lg">
              <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="w-8 h-8 flex items-center justify-center">-</button>
              <span className="w-8 text-center text-sm font-bold">{quantity}</span>
              <button onClick={() => setQuantity((q) => q + 1)} className="w-8 h-8 flex items-center justify-center">+</button>
            </div>
             {currentVariant && <span className="text-xs text-gray-500">(Còn {currentVariant.stock_qty} sản phẩm)</span>}
          </div>
