...
export default function Home() {
  ...
  useEffect(() => {
    const handleSocialLogin = async () => {
      // Kiểm tra URL có chứa ?login_social=success không
      if (searchParams.get("login_social") === "success") {
        try {
          // Gọi API /me để lấy thông tin user (Cookie đã có sẵn trong trình duyệt)
          const userRes = await authService.getMe();

          if (userRes) {
            // Nếu có user
            // 1. Lưu vào localStorage
            // Lưu ý: userRes.data hay userRes tùy thuộc cấu trúc trả về của hàm getMe ở bước 1
            const userData = userRes.data || userRes;
            localStorage.setItem("user_info", JSON.stringify(userData));

            // 2. Dispatch sự kiện để Header cập nhật avatar
            window.dispatchEvent(new Event("auth:changed"));

            // 3. Xóa query param trên URL cho đẹp
            router.replace("/", { scroll: false });

            // (Optional) Hiển thị thông báo chào mừng nhỏ nếu bạn có thư viện toast
            // toast.success(`Chào mừng ${userData.name}!`);
          }
        } catch (err) {
          console.error("Lỗi lấy thông tin user sau khi social login", err);
        }
      }
    };
              ...
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[1, 2, 3, 4].map((i) => (
                    <div
                      key={i}
                      className="h-[350px] bg-gray-200 dark:bg-gray-800 rounded-lg animate-pulse"
                    ></div>
                  ))}
                </div>
              ) : error ? (
